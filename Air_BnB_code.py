...
for review in reviews:
        ...
        new_dfs.append(new_row.to_frame().T)  # Reshape to single-row DataFrame

    # Concatenate the new DataFrames (efficient for multiple rows)
if new_dfs:  # Check if there are any reviews to avoid empty concatenation
        expanded_df = pd.concat(new_dfs, ignore_index=True)
else:
        expanded_df = df.copy()  # If no reviews, keep the original DataFrame
